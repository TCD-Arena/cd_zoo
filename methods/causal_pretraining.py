...
def run_causal_pretraining(X,cfg):
    

    
    # Wrapper to catch this case as it isnt possible as of right now.
    if X.shape[0] > 5: 
        print("RETURN NULL MODEL as we have more than 5 variables")
        return np.zeros((X.shape[0],X.shape[0],3)), None
    ...
    model = Architecture_PL.load_from_checkpoint(cfg.weight_path + cfg.architecture + ".ckpt", weights_only=False)
    ...
    sample_prep = pad(torch.Tensor(X).unsqueeze(0).to("cuda"))
    #Has to be provided.
    ...
    # reverse to put the first lag first
    pred = np.flip(pred, axis=2)
    return pred, None #  No instant available
