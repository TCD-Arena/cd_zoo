...
class transformer(nn.Module):
    ...
    def forward(
        ...
        a = self.fc1(inp)
        b = self.norm(a)
        hidden1 = self.activation(b)

        hidden2 = self.fc2(hidden1)
        if self.regression_head:
            reg_out = self.fc3(hidden1)
            if torch.isnan(reg_out).sum() > 0:
                reg_out = torch.nan_to_num(reg_out,nan=0)
        if torch.isnan(hidden2).sum() > 0:
            hidden2 = torch.nan_to_num(hidden2,nan=0)
        if self.regression_head:
